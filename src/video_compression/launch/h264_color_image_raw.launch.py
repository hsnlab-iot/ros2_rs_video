...
def generate_launch_description():
    return LaunchDescription([
        Node(
            ...
            remappings=[
                ('/video/h264', '/video/color/h264'),  # Remap the input topic for compressed H.264 video
                ('/video/image_raw', '/robot/camera/image_raw'),  # Remap the output topic for raw images
                ('/video/camera_info', '/robot/camera/camera_info')  # Remap the output topic for camera info
            ]
        )
    ])